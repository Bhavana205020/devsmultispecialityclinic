    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage consultant profiles and JPEG photos.</p>
        </div>
        <button
          onClick={() => setEditing({ ...empty })}
          className="btn-gold rounded-md px-4 py-2 text-sm font-semibold inline-flex items-center gap-2"
        >
          ...
          <div key={d.id} className="bg-card border border-border rounded-xl p-4 shadow-card">
            <div className="flex items-start gap-3">
              <div className="w-16 h-16 rounded-full overflow-hidden bg-soft border-2 border-gold/30 flex items-center justify-center shrink-0">
                {d.photo_url ? (
                  ...
                  <UserRound className="h-8 w-8 text-brand/40" />
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-brand truncate">{d.name}</p>
                <p className="text-xs text-muted-foreground line-clamp-2">{d.title}</p>
                <p className="text-xs mt-1 inline-block px-2 py-0.5 rounded bg-gold/15 text-gold-foreground">
                  {d.specialty}
                </p>
              </div>
            </div>
            <div className="flex gap-2 mt-3">
              <button
                ...
                className="flex-1 text-xs py-1.5 rounded border border-border hover:bg-soft inline-flex items-center justify-center gap-1"
              >
                <Pencil className="h-3 w-3" /> Edit
              </button>
              <button
                onClick={() => remove(d.id)}
                className="flex-1 text-xs py-1.5 rounded border border-destructive/30 text-destructive hover:bg-destructive/10 inline-flex items-center justify-center gap-1"
              >
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-20 h-20 rounded-full overflow-hidden bg-soft border-2 border-gold/30 flex items-center justify-center">
                  {editing.photo_url ? (
                    <img src={editing.photo_url} className="w-full h-full object-cover" alt="" />
                  ) : (
                    <UserRound className="h-9 w-9 text-brand/40" />
                  )}
                </div>
                <div className="flex-1">
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="inline-flex items-center gap-2 text-sm px-3 py-2 rounded border border-border hover:bg-soft disabled:opacity-50"
                  >
              ...
              <Field label="Specialty">
                <input className={inputCls} value={editing.specialty} onChange={(e) => setEditing({ ...editing, specialty: e.target.value })} />
              </Field>
              <Field label="Qualifications">
                <textarea rows={3} className={inputCls} value={editing.qualifications ?? ""} onChange={(e) => setEditing({ ...editing, qualifications: e.target.value })} />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Display Order">
                  <input type="number" className={inputCls} value={editing.display_order} onChange={(e) => setEditing({ ...editing, display_order: Number(e.target.value) })} />
                </Field>
                <Field label="Active">
